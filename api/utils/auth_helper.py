# F:\vue_flask_project\vue_flask_project_one\flask\pythonProject1\api\utils\auth_helper.py
import time
    ...
    def encode_auth_token(user_id,login_time):
        ...
        try:
            payload = {
                ...
                'iss':'flask_test(this is barry test)',
                'data':{
                    'id':user_id,
        ...
        if not user: # 用户不存在
            ...
            if user.check_password(password): # 密码正确
                ...
                try:
                    ...
                    if update_result is not True: # 更新失败
                ...
                # token = str(token,encoding='utf-8')      # 旧版本 PyJWT 的返回值是 bytes
                # 因为新版本的 PyJWT 返回的是字符串，但代码还在尝试用 bytes 的方式解码。说以这一行字符转化注释掉
                if token is None:
                    return error(code = HttpCode.auth_error,msg = '生成token失败')
